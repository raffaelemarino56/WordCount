...
int main (int argc, char *argv[])
{
    ...
    if (rank == 0) {
        ...
        MPI_Recv(parole, arrayAppoggio[rank] , wordtype, source, tag, MPI_COMM_WORLD, &stat);
        contaOccorrenze(parole,arrayAppoggio[rank]);
        MPI_Send(parole, arrayAppoggio[rank] , wordtype, 0, tag, MPI_COMM_WORLD);
    }
